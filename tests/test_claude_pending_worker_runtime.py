...
from kanban_adapter import claude_pending_final as queue, compatibility, conversation_runtime
...
def test_real_nonblocking_contention_retries_without_renewing_budget(tmp_path, monkeypatch, persistent):
    ...
    monkeypatch.setattr(service, 'clock_ns', lambda: initial['expires_ns'] - queue.TTL_NS + int(elapsed[0] * 1e9))
