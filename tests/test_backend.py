...
def test_update_with_an_idempotency_key_appends_when_the_marker_is_absent() -> None:
    ...
    assert runner.calls[0] == [
        "hermes", "kanban", "--board", "project-a", "show", "--json",
        "--", "t_abc123",
    ]
...
def test_update_with_an_idempotency_key_skips_an_already_posted_comment() -> None:
    runner = ShowRunner(comments=[f'Claude Code tool usage\n{{"event_id": "{MARKER}"}}'])
    backend = HermesCliBackend(runner=runner)

    backend.update(
        board="project-a", task_id="t_abc123", message="x", idempotency_key=MARKER
    )

    assert [call for call in runner.calls if "comment" in call] == []


def test_update_finds_the_marker_wherever_the_card_schema_puts_it() -> None:
    runner = ShowRunner()
    runner.show_stdout = json.dumps(
        {"task": {"activity": [{"kind": "comment", "text": f"usage {MARKER} here"}]}}
    )
    backend = HermesCliBackend(runner=runner)
...
def test_update_refuses_to_append_when_the_card_cannot_be_inspected(stdout) -> None:
    """Unverifiable means unappendable: a duplicate comment is not acceptable."""
    runner = ShowRunner(show_stdout=stdout)
    backend = HermesCliBackend(runner=runner)
