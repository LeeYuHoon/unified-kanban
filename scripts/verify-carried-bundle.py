#!/usr/bin/env python3
"""Verify carried-bundle checksum, size, prerequisite, and ordered refs."""
...
class DuplicateMetadataKey(ValueError):
    """Raised when JSON metadata repeats a key."""


def _verify_pinned_updater_argv(source: str) -> None:
    """Reject a pinned Hermes updater whose delegated Git argv contract drifted."""
    required = (
...
def _read_regular(path: Path) -> bytes:
    """Read a stable regular file without following a leaf symlink."""
...
def _verify_with_git(
    ...
    *,
    ...
    """Unbundle into a private clone so Git validates object and delta integrity."""
...
def verify(patches: Path, *, hermes_repo: Path | None = None) -> dict[str, str | int]:
    """Return actual metadata after rejecting every undeclared manifest/header entry."""
    ...
    # Git bundle v2 packfiles use SHA-1 as their format checksum. This is an
    # integrity-format comparison, not a security decision or trust anchor.
...
def main(argv: list[str] | None = None) -> int:
    """Fail unless all redundant carried-bundle integrity records agree."""
