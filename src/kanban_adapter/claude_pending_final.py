"""Claude file-v2/부재 시작: 원문 없는 인증 작업과 유한 final 재시도."""
...
import sys
from .claude_file_provenance import seal
from . import claude_absent as absent
...
def _mac(service, body):
    return hmac.new(service.bindings.secret, b'claude-pending-final-v1\0' + _wire(body), hashlib.sha256).hexdigest()


def _key(kwargs):
    task = hashlib.sha256(_wire([kwargs['board'], kwargs['task']])).hexdigest()
    turn = hashlib.sha256(_wire([kwargs['board'], kwargs['task'], kwargs['prompt_id'], kwargs['task_receipt']['nonce']])).hexdigest()
    return task + '.' + turn
    ...
    with _locked(path) as directory:
        ...
        try:
            ...
            if service.clock_ns() >= body['expires_ns'] or body['attempts'] >= MAX_ATTEMPTS:
                ...
                try:
                    kwargs = body['kwargs']
                    if kwargs['prepared'].get('mode') == 'claude-absent-v1':
                        pin = body.get('first_open')
                        if pin is None:
                            # 부재 대기는 open하지 않는다. 인증/조상을 확인한 경로의 존재 여부만 본다.
                            _, _, _, locator = absent._final_authorize(
                                service, mode='claude-absent-v1', **kwargs)
                            try:
                                absent._ancestry(locator, expected=kwargs['prepared']['ancestors'])
                                (locator.allowed_root / locator.relative_path).lstat()
                            except FileNotFoundError:
                                absent._ancestry(locator, absent=True, expected=kwargs['prepared']['ancestors'])
                                return 'pending'
                            # 최초 open 전에 재사용 금지를 CAS 저장한다. 중단/저장 실패는 fail-closed다.
                            body['status'] = 'rejected'
                            receipt = _save(service, path, body, directory, receipt)
                            pin = absent.prepare_final(service, **kwargs)
                            body['first_open'] = pin
                            body['status'] = 'pending'
                            receipt = _save(service, path, body, directory, receipt)
                        # 원래 부재 증거는 중복 enqueue 비교용일 뿐, 봉인에는 저장된 pin만 쓴다.
                        result = absent.seal_final(service, **{**kwargs, 'prepared': pin},
                                                   reconcile_existing=True)
                    else:
                        result = seal(service, **kwargs, require_final=True, reconcile_existing=True)
                    body['status'] = result['status'] if result['status'] in {'pending', 'ready'} else 'rejected'
                except (OSError, ValueError, RuntimeError, KeyError, TypeError) as error:
                    from .claude_hook_entry import report_diagnostic
                    report_diagnostic('stop', 'conversation-preserve-failed', exception=error)
                    body['status'] = 'rejected'
            receipt = _save(service, path, body, directory, receipt)
            return body['status']
        finally:
...
def launch(path):
    ...
    entry = ('import runpy,sys; sys.path.insert(0,sys.argv.pop(1)); '
             'runpy.run_module("kanban_adapter.claude_pending_final",run_name="__main__")')
