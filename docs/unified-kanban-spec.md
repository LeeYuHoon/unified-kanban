...
hermes kanban boards create project-a --name "Project A" --icon 🚀
hermes kanban boards create project-b --name "Project B" --icon 🔧
...
- **업데이트/호환**: `patches/hermes-agent-carried-commits`와
  ...
  `hermes version`의 upstream SHA, `~/.hermes/hermes-agent` 체크아웃,
  `patches/hermes-agent-supported-upstream`의 검증된 full SHA 일치와 carried commit
  존재를 검증한다. pin 경로는 환경변수로 대체할 수 없고, no-follow descriptor open과
  lstat/fstat inode 일치 검사로 읽는다. pin mismatch 또는 malformed pin이면 어떤 설치
  write도 하지 않고 중단한다. updater는 fetch된 `origin/main`이 pin과 다르면 checkout을
  변경하기 전에 중단하며, upstream이 이미 최신이어도 carried commit이 빠져 있으면 REPAIRING
  경로로 setup을 재실행한다(회귀 유지). Hermes Agent 체크아웃이 없으면 호환성을
  검증할 수 없으므로 setup은 Claude/Codex를 포함한 모든 설치 write 전에 중단한다. 이미
  로드된 Hermes plugin도 등록 시점과 각 새 turn 시작에서 pin, `origin/main`, active `HEAD`,
  carried manifest의 최종 commit, `hermes version`이 보고한 upstream을 다시 검사하고, 설치된
  Claude/Codex hook과 `kanban-adapter` wrapper 및 module CLI도 매 실행 전에 검사한다. direct
