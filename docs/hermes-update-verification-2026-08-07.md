...
> **Historical snapshot.** 이 문서의 upstream, carried commit 수, bundle checksum 및 테스트 수는
> 2026-08-07 후보에 대한 기록이다. 이후 후보의 publication 근거로 재사용하지 않는다. 현재 값은
> `patches/hermes-agent-supported-upstream`, `patches/hermes-agent-carried-commits`,
> `patches/hermes-agent-carried-bundle-metadata.json`과 새 테스트 실행에서 확인한다.
...
5. 당시 applied-state의 CRLF 종료를 정상화하고 `origin/main` 기반 repair를 fail-closed하도록 했다. 이 moving-ref gate는 이후 frozen exact-SHA release lifecycle로 대체됐다.
...
7. `patches/hermes-agent-supported-upstream`에 검증된 full upstream SHA를 고정했다. pin은 환경변수로 대체할 수 없고 no-follow descriptor와 inode 재검증으로 읽는다. 당시 setup/updater는 checkout ref equality를 사용했지만, 현재는 fixed official HTTPS 저장소에서 frozen exact SHA와 reviewed bundle을 검증하고 moving `main`은 다음 cycle로 넘긴다. 설치된 Hermes plugin, Claude/Codex hook, adapter도 selected release authority mismatch를 차단한다.
...
- Unified Kanban: 654 passed in 82.19s
...
| `claude-fable-5` | 압축 diff 기반 독립 리뷰 | 당시 성공. CRLF state와 moving-ref 기반 gate 문제를 발견·수정했으며, 이 gate는 현재 frozen exact-SHA lifecycle로 대체됨 |
