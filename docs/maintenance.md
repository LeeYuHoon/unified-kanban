...
Deletion is a separate macOS-only action using the same command with `--apply`. Apply holds `$HERMES_HOME/state/hermes-kanban-update.lock`, retains descriptor and inode authority for the release root, and protects `current` and `previous` together with references found in the configured or loaded launchd service and stable same-UID runtime process and open-file probes. An unavailable, malformed, or non-convergent reference probe prevents deletion of the affected candidate and aborts subsequent work; candidates already deleted earlier in the same apply run are not restored. If a later failure follows a completed deletion or retained-state decision, the CLI emits deterministic partial-result JSON identifying those completed deletions and retained release-state artifacts before returning nonzero. A pre-mutation failure with durably absent recovery state has no partial state to report and returns nonzero without this JSON. If retirement-record publication cleanup cannot be made durable, the CLI instead reports the record path as indeterminate retained state because a crash may resurrect it.

A candidate is first moved descriptor-relatively to a fresh private retirement name and bound to a durable retirement record. References and the shared lock are checked again before deletion; the retained release-root capability is revalidated after runtime probes and lock verification immediately before descriptor-relative deletion. A newly referenced release is restored only when its canonical pathname is still absent and owned by that operation. On crash recovery, the record, retirement identity, complete receipt and contents, and fresh runtime references are revalidated before deletion resumes. A partial or tampered retirement is retained with its record and is neither deleted nor restored to the canonical pathname. Foreign successors, replacement roots, malformed records, and indeterminate recovery state are preserved for deliberate inspection.
