...
Setup의 pre-bootstrap path normalization은 ambient Python에 의존하지 않으며, absent checkout/data/release path ancestry도 private temp 생성이나 download 전에 검증합니다. `before installer` 단계에서는 root-owned macOS `system Perl`의 full ancestry·leaf authority를 검증합니다. 불완전한 receipt bytes는 `staging-only` namespace에 두고 durable write가 끝난 뒤 recoverable candidate namespace로 atomic arm하므로, installer가 실행된 모든 실패 상태에 retry authority가 남으며 pre-arm crash artifact는 안전하게 정리됩니다. `HERMES_AGENT_REPO`와 `HERMES_HOME`의 root-equivalent spelling은 `filesystem root`로 normalize되는 즉시 어떤 bootstrap write보다 먼저 거부합니다. Frozen installer가 만드는 `venv/bin/python` `absolute symlink`는 검증된 system Perl로 `raw symlink target bytes`를 읽고 managed runtime 내부로 confinement한 뒤 symlink identity와 resolved target의 full ancestry·leaf·stable identity를 검증합니다. Bootstrap receipt를 공개하거나 재사용하기 전에 managed Python 3.11 이상과 managed Node 26을 fixed paths에서 직접 실행하고 intermediate ancestry와 managed uv parent까지 검증합니다. Receipt file과 state directory를 `fsync`하며, 새 bootstrap state/release root를 만들 때는 각 새 directory entry의 containing parent도 `fsync`하고, retry에서는 기존 private component의 containing parent를 다시 `fsync`해 미완료 durability를 repair합니다. Crash가 publication 전 private candidate만 남기거나 canonical receipt와 same-inode private candidate를 함께 남긴 경우, normal setup이 candidate authority를 감지해 status recovery를 호출합니다. Recovery는 exact bytes와 identity/toolchain을 인증하고 candidate file을 `fsync`한 뒤 publication 또는 candidate cleanup을 durable하게 재개합니다. Existing release root는 containing-parent `fsync` 전에 먼저 검증하며, managed launcher는 selector의 complete stable bytes를 읽었음을 확인합니다. Existing checkout, release root, data home의 `/`부터 이어지는 ancestry에 symlink, foreign owner, group/other `writable ancestor`가 있거나 managed selector/executable leaf의 owner·mode·link count가 안전하지 않으면 setup과 managed launcher 모두 fail closed합니다.
