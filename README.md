...
- Unified Kanban release commit: `898adeb837d4879df80189e1b5f6dcbf97542649`
...
보드 선택 위에는 **전체기간 누적**, 기존 요약 자리에는 **선택일 실측 사용량**을 표시합니다. 작은 달력의 기본은 한국 시간(KST) 오늘입니다. 날짜를 바꿔도 카드를 숨기지 않으며 누적값은 그대로입니다.

두 요약 모두 전체·입력·캐시·출력·추론과 실제 모델별 토큰·카드 수를 보여 줍니다. 모델은 GPT·Claude·모델미상·기타모델로 구분하며 도구 이름으로 추측하지 않습니다. 누적과 선택일은 서로 겹치므로 더하지 않습니다.

실제 사용시각이 기록된 수집분만 표시합니다. **N/A**는 확인할 수 없다는 뜻이며 0과 다릅니다. 과거 기록의 생성일 추정량은 실측에 합치지 않습니다. 기존 카드·보드 누적 사용량은 날짜와 무관합니다.

토큰은 **K(천)·M(백만)·B(십억)** 단위로 표시합니다. Claude도 누적 사용량에는 남지만, 사용시각이 없으면 선택일 실측에는 포함되지 않습니다.

자세한 기준은 [날짜별 토큰 안내](docs/daily-search.md)를 보세요. 이 달력 UI는 검증 중인 후보이며 운영에는 아직 적용하지 않았습니다.
