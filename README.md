...
구성: Python 3.11 CLI 어댑터 + Claude/Codex 훅 실행 파일 + Hermes Agent 플러그인 + Bash 설치·업데이트·스모크 스크립트. 핵심 통합 계층은 테스트 495개로 검증하며, 줄 수처럼 쉽게 낡는 지표보다 module 책임과 실제 검증 범위를 문서화합니다.
...
**7. Hermes는 릴리스 주기가 빠르고, 업데이트가 local commit을 되돌린다.** Dashboard의 토큰 집계·observation 지원은 Hermes 체크아웃에 얹는 carried commit이라 `hermes update` 후 사라질 수 있고, 그러면 기록이 조용히 끊깁니다. setup은 어떤 write보다 먼저 설치된 CLI의 upstream SHA와 체크아웃 일치, 저장소가 검증한 `patches/hermes-agent-supported-upstream`의 정확한 SHA, carried commit 적용 여부, 그리고 `--help` 출력에 어댑터가 의존하는 플래그가 모두 있는지를 검사하고 실패하면 중단합니다. 업데이터는 upstream이 이미 최신이어도 빠진 carried commit을 복구합니다.
...
- **자동화 테스트** — 저장소 테스트 495개(`tests/`, 19개 모듈), 세션 뷰어 테스트 159개를 합쳐 654개. 커버 범위는 어댑터 CLI와 백엔드, Claude/Codex/Hermes 훅 수명주기, 자동 위임·백그라운드 알림 제외, 사용량·토큰 정규화, Codex 모델 폴백, 훅 설치기, setup/uninstall, Hermes 플러그인 등록과 업데이터입니다.
...
- **설치 전 호환성 검사** — setup이 `hermes kanban --help` 계열 출력에서 `--observation`, `--tenant`, `--created-by`, `--initial-status`, `--json` 등 의존 플래그의 존재를 토큰 단위로 확인하고, CLI/checkout/pinned upstream SHA와 carried commit 적용 여부를 검사한 뒤에만 파일을 씁니다.
...
2. 설치된 Hermes CLI의 upstream SHA와 `~/.hermes/hermes-agent` checkout이 일치하는지 확인합니다.
3. checkout SHA가 이 저장소에서 검증한 `patches/hermes-agent-supported-upstream`의 정확한 SHA와 일치하는지 확인하고, 다르면 어떤 설치 파일도 쓰기 전에 중단합니다.
4. `patches/hermes-agent-carried.bundle`에서 필요한 commit object를 import하고, 누락된 carried commit이 있으면 Hermes를 준비한 뒤 설치를 계속합니다.
...
9. 실제 Hermes Kanban에서 카드 생성 → 코멘트 → 완료 → archive를 검증하는 smoke test를 실행합니다. 기본 smoke 보드는 `unified-kanban-smoke`이며 테스트 카드는 마지막에 archive됩니다.
...
| `Setup blocked: unified-kanban supports Hermes upstream ...` | 설치된 Hermes가 이 저장소에서 검증한 SHA보다 새롭거나 다릅니다. setup을 우회하지 말고 upstream 변경 분석·carried 회귀 테스트를 통과한 뒤 `patches/hermes-agent-supported-upstream`을 갱신하세요. |
| `refusing dirty Hermes checkout` | Hermes checkout의 사용자 변경을 먼저 commit/stash/revert한 후 다시 실행. 업데이터는 `--force`로도 dirty checkout을 덮어쓰지 않음 |
...
설치는 파일을 쓰기 전에 설치된 Hermes CLI가 `~/.hermes/hermes-agent` 체크아웃의 upstream SHA와 일치하는지, checkout이 `patches/hermes-agent-supported-upstream`의 검증된 SHA와 정확히 일치하는지, `patches/hermes-agent-carried-commits`의 모든 커밋이 적용됐는지 확인합니다. pin 경로는 환경변수로 바꿀 수 없고 파일은 symlink를 따르지 않는 descriptor로 열어 inode를 재검증합니다. updater도 fetch한 `origin/main`이 pin과 다르면 `hermes update` 전에 중단합니다. 이미 설치한 뒤 사용자가 직접 `hermes update`한 경우에도 Hermes plugin은 등록 시점과 매 새 turn 시작 전에, 설치된 Claude/Codex hook과 `kanban-adapter`는 매 실행 전에 pin, `origin/main`, active `HEAD`, carried manifest 최종 commit, `hermes version`의 upstream이 모두 일치하는지 검사합니다. mismatch면 hook은 외부 CLI를 방해하지 않도록 기록 없이 종료하고 adapter는 오류로 중단하므로 새 버전에서 이 프로젝트를 계속 적용하지 않습니다.
...
업데이터는 upstream을 fetch하고, 변경이 있을 때만 `hermes update`를 실행한 뒤 carried commit을 다시 적용합니다. upstream이 이미 최신이어도 누락된 carried commit은 복구합니다. 이후 setup을 다시 실행하고, 기존에 Dashboard가 실행 중이었다면 재시작합니다. Hermes Agent 체크아웃이 없으면 호환성을 검증할 수 없으므로 설치를 중단합니다.
...
보드, 카드, 프로젝트 metadata를 삭제하지 않고 설치된 링크, Hermes plugin 심볼릭 링크와 레거시 관리형 routing 항목을 제거합니다. `hermes-kanban` plugin도 비활성화합니다.
